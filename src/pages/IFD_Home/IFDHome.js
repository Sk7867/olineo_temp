...
import useOnScreen from "../../hooks/useOnScrenn";
...
export const IFDHome = ({ setHeaderData, userLoggedIn }) => {
  ...
  const navigateToRewardPage = () => {
    gsap.set("#home-button-span", { opacity: 1 });
    tl.to("#home-button-span", {
      scale: 80,
      opacity: 0.05,
      duration: 1,
      delay: 0.1,
      onComplete: () => {
        return setHomePage(false), tl.kill();
      },
    });
    tl.to("#poster-container", { opacity: 0, duration: 1 }, "<");
    tl.to("#thumbnail-container", { opacity: 0, duration: 1 }, "<");
    tl.to("#primary-floating-button", { opacity: 0, duration: 0.6 }, "<");
  };

  useEffect(() => {
    if (!homePage) return;
    gsap.fromTo("#poster-container", { y: 50, opacity: 0 }, { y: 0, opacity: 1, duration: 1.3, delay: 0.2, ease: "back.out(1)" });
    gsap.fromTo("#thumbnail-container", { left: -60 }, { left: 15, duration: 1.2, delay: 1.3, ease: "back.out(1)" });
    gsap.fromTo(
      ...
      <div className={`${styles["container"]} `}>
        {homePage ? (
          <>
            ...
            <svg style={{ position: "absolute", bottom: "30px", right: "20px" }} width={45} height={106} viewBox="0 0 45 106" fill="none" xmlns="http://www.w3.org/2000/svg">
              <g id="Group">
                <g onClick={() => navigate("/store-finder")} style={{ cursor: "pointer" }} id="discount-offer">
                ...
                <g onClick={() => navigate("/store-finder")} style={{ cursor: "pointer" }} id="store-finder">
          ...
          <IFD setHomePage={setHomePage} userLoggedIn={userLoggedIn} />
        )}
      </div>
